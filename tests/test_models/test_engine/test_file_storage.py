#!/usr/bin/python3
"""Defines unittests for the FileStorage clas in the file_storage module
"""
import os
import models
import unittest
...
class TestFileStorage(unittest.TestCase):
    """Unittests for instantiating the FileStorage class."""

    def test_FileStorage_with_no_args(self):
    ...
    def test_storage_(self):
        self.assertEqual(type(models.storage), FileStorage)


class TestFileStorage_test_methods(unittest.TestCase):
    ...
    def test_new_method_with_args(self):
        """ Test case for the new() method with arguments"""
        with self.assertRaises(TypeError):
            models.storage.new(BaseModel(), 1)

    def test_new_method_with_None(self):
        """ Test case for the new() method with no None value"""
    ...
    def test_save_method(self):
        ...
        with open("file.json", "r") as content:
            save_content = content.read()
            self.assertIn("BaseModel." + base.id, save_content)
            self.assertIn("User." + user.id, save_content)
            self.assertIn("State." + state.id, save_content)
            self.assertIn("Place." + place.id, save_content)
            self.assertIn("City." + city.id, save_content)
            self.assertIn("Amenity." + amenity.id, save_content)
            self.assertIn("Review." + review.id, save_content)
